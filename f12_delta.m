...
function out = f12_delta(in, R, Rl, Xl, Xarm, vhvdc, vgrid, pconu, pconl, sgrid, idcdif_ref, reiacsum_ref)
    ...
    eqn(9) = (-vdcdif + vdcsum/2)*(idcdif/2 + idcsum) - revacdif*(reiacdif/2+reiacsum) - imvacdif*(-imiacdif/2-imiacsum) + 0.5*revacsum*(reiacdif/2+reiacsum) + 0.5*imiacsum*(-imiacdif/2-imiacsum) - pconu;
    eqn(10) = (vdcdif + vdcsum/2)*(-idcdif/2 + idcsum) + revacdif*(-reiacdif/2+reiacsum) + imiacdif*(imiacdif/2-imiacsum) + 0.5*revacsum*(-revacdif/2+reiacsum) + 0.5*imiacsum*(imiacdif/2-imiacsum) - pconl;
